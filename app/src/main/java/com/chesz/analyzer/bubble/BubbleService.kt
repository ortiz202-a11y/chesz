            ...
            container.setOnTouchListener { _, e ->
                ...
                when (e.action) {
                    ...
                    MotionEvent.ACTION_MOVE -> {
                        val newX = oX + (e.rawX - dX).toInt()
                        val newY = oY + (e.rawY - dY).toInt()
                        
                        // LÍMITES REALES: El botón ya no se frena antes de la orilla
                        lp.x = newX.coerceIn(0, screenWidth - container.width)
                        lp.y = newY.coerceIn(0, screenHeight - container.height)
